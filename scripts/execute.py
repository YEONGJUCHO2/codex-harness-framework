...
Runs phase steps through implementation Codex sessions. Implementation sessions
own the deterministic lint/build/test gate and submit verified work for
completion. The executor owns final step completion metadata and commits.
"""
...
class StepExecutor:
    """Executes phase steps with implementation and phase-evaluation gates."""
    ...
    STEP_BLOCKING_STATUSES = {"error", "blocked", "evaluation_failed", "review_failed"}
    EVAL_DECISIONS = {"approved", "changes_requested", "blocked"}
    ...
    def _update_top_index(self, status: str):
        ...
        ts_key = {
            ...
            "error": "failed_at",
            "blocked": "blocked_at",
            "evaluation_failed": "evaluation_failed_at",
    ...
    def _build_implementation_preamble(
        ...
        *,
        ...
        return (
            ...
            "Implement the requested step, run the official verification commands, "
            "then submit it for executor completion.\n\n"
            f"{guardrails}\n\n---\n\n"
            ...
            "3. Run the official verification commands before finishing:\n"
            "   ```bash\n"
            "   npm run lint\n"
            "   npm run build\n"
            "   npm run test\n"
            "   ```\n"
            "4. If this repository has no package.json or uses different step AC commands, "
            "run the applicable commands and name them in the summary.\n"
            "5. Do not set this step to completed. Only scripts/execute.py may set completed.\n"
            f"6. Update /phases/{self._phase_dir_name}/index.json for this step:\n"
            "   - Verified completion: set \"status\": \"ready_for_completion\" and add a concise \"summary\" with verification results.\n"
            "   - Needs user input: set \"status\": \"blocked\" and add \"blocked_reason\".\n"
            "   - Cannot produce verified work after self-correction: set \"status\": \"error\" and add \"error_message\".\n"
            "7. 커밋하지 마라. Do not commit. execute.py가 커밋을 담당하며 scripts/execute.py owns commit creation.\n\n---\n\n"
        )
    ...
    def _check_blockers(self):
        ...
        for s in reversed(index["steps"]):
            ...
            if status == "error":
                ...
                sys.exit(1)
            if status in {"evaluation_failed", "review_failed"}:
                print(f"\n  x Step {s['step']} ({s['name']}) failed evaluation.")
                print(f"  Reason: {s.get('evaluation_summary') or s.get('review_summary', 'unknown')}")
                print("  Address the finding and reset status to 'pending' to retry.")
                sys.exit(1)
            if status == "blocked":
    ...
    def _read_decision_report(self, path: Path, *, fallback_step: dict, cycle: int) -> dict:
        if not path.exists():
            return {
                ...
                "summary": f"Decision report was not created at {self._relative(path)}.",
                "commands": [],
                "findings": [
                    {
                        "severity": "blocker",
                        ...
                        "line": None,
                        "message": "The required decision JSON report was not produced.",
                    }
                ],
                "required_changes": ["Produce the required decision report."],
            }
        ...
        try:
            ...
            return {
                ...
                "cycle": cycle,
                "decision": "changes_requested",
                "summary": f"Decision report is not valid JSON: {exc}",
                "commands": [],
                "findings": [
                    {
                        "severity": "blocker",
                        "file": self._relative(path),
                        "line": None,
                        "message": "Decision report could not be parsed.",
                    }
                ],
                "required_changes": ["Write a valid JSON decision report."],
            }

        decision = report.get("decision")
        if decision not in self.EVAL_DECISIONS:
            ...
            report.setdefault("findings", []).append(
                {
                    ...
                    "file": self._relative(path),
                    "line": None,
                    "message": f"Invalid decision: {decision!r}.",
                }
            )
    ...
    def _run_implementation_until_ready(
        ...
        *,
        ...
        for attempt in range(1, self.MAX_IMPLEMENTATION_ATTEMPTS + 1):
            ...
            output_path = self._outputs_dir / f"step{step_num}-implementation-attempt{attempt}.json"
            ...
            if status == "completed":
                ...
                current["completion_override"] = "Implementation agent attempted to mark completed; executor owns final completion metadata."
                self._write_json(self._index_file, index)
                print(f"  ! Step {step_num}: implementation tried to complete; demoted to ready_for_completion [{elapsed}s]")
                return

            if status in {"ready_for_completion", "ready_for_review"}:
                if status == "ready_for_review":
                    current["status"] = "ready_for_completion"
                    current["completion_override"] = "Legacy ready_for_review status treated as ready_for_completion."
                    self._write_json(self._index_file, index)
                print(f"  > Step {step_num}: ready for completion [{elapsed}s]")
                return
            ...
            err_msg = current.get("error_message") or f"Step did not reach ready_for_completion (status: {status})"
    ...
    def _execute_single_step(self, step: dict, guardrails: str) -> bool:
        ...
        if current.get("status") != "ready_for_completion":
            self._run_implementation_until_ready(step=step, guardrails=guardrails)

        index = self._read_json(self._index_file)
        current = self._step_record(index, step_num)
        current["status"] = "completed"
        current["completed_at"] = self._stamp()
        current["verified_by"] = "implementation_agent"
        current.setdefault("summary", f"{step_name} implemented and verified")
        self._write_json(self._index_file, index)
        self._commit_step(step_num, step_name)
        print(f"  OK Step {step_num}: {step_name} completed")
        return True
